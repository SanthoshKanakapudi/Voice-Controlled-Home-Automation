...
def buttonpage():
    ...
    B31=Button(hf,text="ON",padx=5,pady=5,width=30,command=lambda:[turn("v2",1),button_highlight(B31,B32)],font=("bold"),)
    B32=Button(hf,text="OFF",padx=5,pady=5,width=30,command=lambda:[turn("v2",0),button_highlight(B32,B31)],font=("bold"))
...
def voicepage(): 
    ...
    def check(tex):
        ...
        if("turn on fan" in tex or "activate fan" in tex):
            turn("v2",1)
            try:
                sendblue(5)
            except:pass
            return mp3("Ok got it, Turning on fan")
        if("turn off fan" in tex or "fan off" in tex):
            turn("v2",0)
            try:
                sendblue(6)
            except:pass
            return mp3("Ok got it, Turning off fan")
        ...
        if("180 degree" in tex):
            turn("v3",9)
            return mp3("Turning motor to 180 degrees")
        if("90 degree" in tex):
            turn("v3",5)
            return mp3("Turning motor to 90 degrees")
        if("zero degree" in tex):
            turn("v3",0)
            return mp3("Turning motor to 0 degrees")
    ...
    img1 =PhotoImage(file="Voice Control Home Automation\spicon.png")
    img2 =PhotoImage(file="Voice Control Home Automation\images.png")
...
def delpage():
    for page in main.winfo_children():
        page.grid_forget()
def changetime():
    a=int(time.ctime()[11:13])
    if(a<12):
        timelabel.configure(text=time.ctime()[11:20]+' AM')
    if(a>12):
        a=a-12
        timelabel.configure(text=str(a)+time.ctime()[13:20]+' PM')
